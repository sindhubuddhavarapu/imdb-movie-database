...
-- Answer : coutry, worlwide_gross_income, languages, production_company are the 4 columns with NULL values in them
...
select count(distinct id) as India_or_USA_movie_count_2019 
from movie 
...
group by genre order by movie_count desc limit 1;

-- Drama genre has the highest movies produced, with a count of 4285
...
-- list of movie ids that belong to only 1 genre
select movie_id, count(distinct genre) as genre_count 
from genre group by movie_id having genre_count = 1;

-- count of movies that belong to only 1 genre (Answer : 3289 movies)
select count(distinct movie_count.movie_id) as one_genre_movie_count 
from (select movie_id, count(distinct genre) as genre_count 
...
from movie as m join genre as g on g.movie_id = m.id 
group by genre 
...
-- Thriller has 3rd rank with 1484 movies
...
-- Using dense_rank(), top 10 movies based on average rating
select title, avg_rating, dense_rank() over(order by avg_rating desc) as movie_rank
from ratings as r join movie as m on m.id = r.movie_id 
limit 10;

-- Using rank(), top 10 movies based on average rating
...
-- Dream Warrior Pictures & National Theatre Live are the production houses with most number of hit movies
select production_company, count(distinct m.id) as movie_count, 
...
-- All movies from each genre that start with 'The' and have an average rating > 8
select title, avg_rating, genre 
from movie as m 
join genre as g on g.movie_id = m.id
join ratings as r on r.movie_id = m.id
where avg_rating > 8 and title like 'The%' 
order by avg_rating desc, title, genre;
...
-- Count of movies released between 1st April 2018 & 1st April 2019 that have median rating > 8
...
from movie as m join ratings as r on m.id = r.movie_id
group by languages having languages like '%German%' or '%Italian%'
...
select count(*) as id_nulls from names where id is null;
...
-- height, date_of_birth, known_for_movies are the columns with NULL values in the 'names' table
...
-- top 3 genres (having most no. of movies with avg_rating > 8)
select genre as top_3_genres, count(*) as movie_count 
from genre as g inner join ratings as r on r.movie_id = g.movie_id
where avg_rating > 8 
group by genre order by movie_count desc limit 3;


-- top 3 directors in each of the above top 3 genres
select name as director_name, count(*) as movie_count 
from (select count(*) as movie_count, genre 
		from genre as g inner join ratings as r on r.movie_id = g.movie_id
		where avg_rating > 8 group by genre order by movie_count desc limit 3
	) as top_genres join role_mapping as rm on rm.movie_id = m.id
    join names as n on n.id = rm.name_id
...
from role_mapping as rm join movie as m on m.id = rm.movie_id
join ratings as r on r.movie_id = m.id
join names as n on n.id = rm.name_id
...
from movie as m join ratings as r on r.movie_id = m.id
...
from movie as m join ratings as r on r.movie_id = m.id
join role_mapping as rm on rm.movie_id = m.id
join names as n on n.id = rm.name_id
where m.country like '%India%' and rm.category like '%actor%'
...
join role_mapping as rm on rm.movie_id = m.id
join names as n on n.id = rm.name_id
where m.country like '%India%' and rm.category like '%actress%' and languages like '%Hindi%'
group by n.name having movie_count >= 3 limit 5;
...
with thrillers as (select distinct title, avg_rating from movie as m
join ratings as r on r.movie_id = m.id
join genre as g on g.movie_id = m.id
...
from thrillers order by avg_rating desc; 
...
-- query to select top 3 genres based on number of movies
select genre, count(m.id) as movie_count, 
...
-- query to select top 5 movies in each year based on worlwide_gross_income
...
from (select title, year, worlwide_gross_income,
            row_number() over(partition by year order by worlwide_gross_income desc) as movie_rank
		from movie
    ) as Ranked_Movies
where movie_rank <= 5 order by year, movie_rank;
...
join names as n on n.id = rm.name_id
join genre as g on g.movie_id = m.id 
where genre = 'Drama' and rm.category like '%actress%' and avg_rating > 8
group by n.name 
limit 3; 
...
select n.id as director_id, n.name as director_name, 
	...
	round( avg(avg_rating), 2 ) as avg_movies_rating,
...
join role_mapping as rm on rm.movie_id = m.id 
join names as n on n.id = rm.name_id
group by n.id, n.name limit 9;



-- Couldn't write complete queries for questions 17, 19, 26 and 29, hence partially solved them.
